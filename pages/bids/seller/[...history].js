...
const [sellerData, setSellerData] = useState({ 
    bid_id: '', bid_token: '', shop_id: '', buyer: 'Others', seller: 'Others', user: 'Seller'
});   
...
useEffect(() => {
    ...
    if(rp) { 
        setSellerData(state => ({
        ...state, 
        bid_id: bid.bid_id, bid_token: bid.bid_token, shop_id: bid.shop_id, buyer: 'Others', seller: 'Counter', user: 'Seller'
      })); 
    }    
  }, [auction]); 
useEffect(() => {
    ...
    return (
        <Fragment>
            ...
            {auction && auction.length  < 1 ? 
                 'Buyer has not made an offer yet'
            :
                <>
                <AuctionHistory user="seller" history={auction} />
                </>
        }
                 </Col>
             </Row>
             <Modal isOpen={modal} toggle={toggle}>
                <ModalHeader toggle={toggle} close={closeBtn}>Counter</ModalHeader>
                <ModalBody>
                    <AuctionForm toggle={toggle} data={sellerData} />
                </ModalBody>
            </Modal> 
               {/* <Row className="d-flex justify-content-center">
        <Col md="12">
...
SellerBidHistory.getInitialProps = async ({ query }) => {
    return {view: query.view}
  }

export default observer(SellerBidHistory);
